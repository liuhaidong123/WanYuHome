...
//修改开锁密码
public class DeviceSettingLockChangePSDActivity extends MyActivity {
...
//        setContentView(R.layout.activity_device_setting_lock_change_psd);
    }
...
//    http://192.168.1.55:8080/smarthome/mobileapi/equipment/doorPasswd.do?token=9DB2FD6FDD2F116CD47CE6C48B3047EE&equipmentId=6&oldPwd=123&newPwd=123456
//    Method:POST
//    |token        |String   |Y    |令牌
//    |equipmentId  |Long     |Y    |门锁设备编号
//    |oldPwd       |String   |Y    |旧密码
//    |newPwd       |String   |Y    |新密码
    public void Submit(View vi){
        if (checkInput()){
            if (!"".equals(DeviceId)&&!TextUtils.isEmpty(DeviceId)){
                ...
                okhttp.getCall(Ip.serverPath+Ip.interface_changeDoorPsd,mp,okhttp.OK_POST).enqueue(new Callback() {
                    @Override
                ...
                mToast.Toast(con,"无法获取到设备信息，修改失败");
            }

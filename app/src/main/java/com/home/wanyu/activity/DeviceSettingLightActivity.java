...
//灯的设置页面
public class DeviceSettingLightActivity extends MyActivity {
    private String title="客厅灯";
    @BindView(R.id.deviceSetting_light_switch)SwitchButton deviceSetting_light_switch;//开关按钮
    @BindView(R.id.deviceSetting_light_mySeekbar)MySeekBar deviceSetting_light_mySeekbar;//亮度调节按钮
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initChildView(R.layout.activity_device_setting_light);
        setTitle(title);
        unbinder= ButterKnife.bind(this,ChildView);
        ShowChildView(DEFAULTRESID);
        initData();
        getIntentData();
        deviceSetting_light_switch.setSwitchChangerListener(new SwitchButton.switchChangeListener() {
            @Override
        ...
        if (b!=null){
            ...
            if (!"".equals(type)&&!TextUtils.isEmpty(type)){
                if ("1".equals(type)){
                    ...
                    if (room.getState()==1L){
                        deviceSetting_light_switch.setNorState(false);
                    }
                    else {
                        deviceSetting_light_switch.setNorState(true);
                    }
                }
    ...
    private void initData() {
        deviceSetting_light_mySeekbar.setOnSeekListener(new MySeekBar.SeekListener() {
            @Override
            public void seek(float mark, int max,int state) {
//               0初始状态，1按下，2滑动,3抬起后的状态
//                Toast.makeText(con,"当前刻度--"+(int)(mark*max)+"---状态---"+state,Toast.LENGTH_SHORT).show();
                Log.i("当前刻度--","当前刻度--"+(int)(mark*max)+"---状态---"+state);
            }
        });
    }

...
//开关的设置页面
public class DeviceSettingSwitchActivity extends MyActivity {
    String title="插座";
    @BindView(R.id.deviceSetting_switch_switch)SwitchButton deviceSetting_switch_switch;//开关按钮
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initChildView(R.layout.activity_device_setting_switch);
        unbinder= ButterKnife.bind(this,ChildView);
        setTitle(title);
        getIntentData();
        ShowChildView(DEFAULTRESID);
        deviceSetting_switch_switch.setSwitchChangerListener(new SwitchButton.switchChangeListener() {
            @Override
        ...
        if (b!=null){
            ...
            if (!"".equals(type)&&!TextUtils.isEmpty(type)){
                if ("1".equals(type)){
                    ...
                    if (room.getState()==1L){
                        deviceSetting_switch_switch.setNorState(false);
                    }
                    else {
                        deviceSetting_switch_switch.setNorState(true);
                    }
                }
